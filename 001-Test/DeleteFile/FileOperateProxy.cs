...
namespace DeleteFile
{
    ...
    public class FileOperateProxy
    {
        ...
        public static int DeleteFile(string filePath, bool toRecycle, bool showDialog, bool showProgress, ref string errorMsg)
        {
            try
            {
                return ToDelete(filePath, toRecycle, showDialog, showProgress, ref errorMsg);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        /// 删除一组文件。
        /// </summary>
        /// <param name="filePaths">字符串数组，表示一组文件路径</param>
        /// <param name="toRecycle">指示是将文件放入回收站还是永久删除，true-放入回收站，false-永久删除</param>
        /// <param name="showDialog">指示是否显示确认对话框，true-显示确认删除对话框，false-不显示确认删除对话框</param>
        /// <param name="showProgress">指示是否显示进度对话框，true-显示，false-不显示。该参数当指定永久删除文件时有效</param>
        /// <param name="errorMsg">反馈错误消息的字符串</param>
        /// <returns>操作执行结果标识，删除文件成功返回0，否则，返回错误代码</returns>
        public static int DeleteFiles(string[] filePaths, bool toRecycle, bool showDialog, bool showProgress, ref string errorMsg)
        {
            try
            {
                string fName = "";
        ...
        /// 移动一个文件到指定路径下
        /// </summary>
        /// <param name="sourceFilePath">要移动的文件路径</param>
        ...
        public static int MoveFile(string sourceFilePath, string destinationPath, bool showDialog, bool showProgress, bool autoRename, ref string errorMsg)
        {
            try
            {
                return ToMoveOrCopy(wFunc.FO_MOVE, sourceFilePath, destinationPath, showDialog, showProgress, autoRename, ref errorMsg);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        /// 移动一组文件到指定的路径下
        /// </summary>
        /// <param name="sourceFilePaths">要移动的文件路径数组</param>
        /// <param name="destinationPath">移动到的目的路径</param>
        /// <param name="showDialog">指示是否显示确认对话框，true-显示确认对话框，false-不显示确认对话框</param>
        /// <param name="showProgress">指示是否显示进度对话框</param>
        /// <param name="autoRename">指示当文件名重复时，是否自动为新文件加上后缀名</param>
        ...
        public static int MoveFiles(string[] sourceFilePaths, string destinationPath, bool showDialog, bool showProgress, bool autoRename, ref string errorMsg)
        {
            try
            {
                string sfName = "";
                ...
                {
                    sfName += str + "\0";   //组件文件组字符串
                }
                return ToMoveOrCopy(wFunc.FO_MOVE, sfName, destinationPath, showDialog, showProgress, autoRename, ref errorMsg);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        /// 复制一个文件到指定的文件名或路径
        /// </summary>
        /// <param name="sourceFilePath">要复制的文件名</param>
        /// <param name="destinationFilePath">复制到的目的文件名或路径</param>
        /// <param name="showDialog">指示是否显示确认对话框，true-显示确认对话框，false-不显示确认对话框</param>
        /// <param name="showProgress">指示是否显示进度对话框</param>
        /// <param name="autoRename">指示当文件名重复时，是否自动为新文件加上后缀名</param>
        /// <returns>返回移动操作是否成功的标识，成功返回0，失败返回错误代码,-200:表示其他异常</returns>
        public static int CopyFile(string sourceFilePath, string destinationFilePath, bool showDialog, bool showProgress, bool autoRename, ref string errorMsg)
        {
            try
            {
                return ToMoveOrCopy(wFunc.FO_COPY, sourceFilePath, destinationFilePath, showDialog, showProgress, autoRename, ref errorMsg);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        /// 复制一组文件到指定的路径
        /// </summary>
        /// <param name="sourceFilePaths">要复制的文件路径数组</param>
        /// <param name="destinationPath">复制到的目的路径</param>
        /// <param name="showDialog">指示是否显示确认对话框，true-显示确认对话框，false-不显示确认对话框</param>
        /// <param name="showProgress">指示是否显示进度对话框</param>
        /// <param name="autoRename">指示当文件名重复时，是否自动为新文件加上后缀名</param>
        /// <returns>返回移动操作是否成功的标识，成功返回0，失败返回错误代码,-200:表示其他异常</returns>
        public static int CopyFiles(string[] sourceFilePaths, string destinationPath, bool showDialog, bool showProgress, bool autoRename, ref string errorMsg)
        {
            try
            {
                string sfName = "";
                ...
                return ToMoveOrCopy(wFunc.FO_COPY, sfName, destinationPath, showDialog, showProgress, autoRename, ref errorMsg);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        public static int ReNameFile(string sourceFilePath, string destinationFilePath, bool showDialog, ref string errorMsg)
        {
            ...
            try
            {
                ...
                lpFileOp.pFrom = sourceFilePath + "\0\0";         //将文件名以结尾字符"\0\0"结束
                lpFileOp.pTo = destinationFilePath + "\0\0";
                ...
                errorMsg = string.Format("{0}({1})", tmp, sourceFilePath);
